...
\section*{Introduction to Mathematical Modeling in Epidemiology}
TC Porco. University of California, San Francisco
...
as software which can be used for estimation.  Our discussion may
...
\section*{Lecture I}
The key ideas in Lecture I will 

\subsection*{Probability Review}

A large number of books and websites can provide you with a probability review.
I recommend the book {\it An elementary introduction to the theory of probability}
by Gnedenko and Khinchin.
...
relation corresponds to what the logicians call ``material implication''.  
But this is only saying that whenever the number of spots is no more than
...
{\bf Exercise.~~}Prove that for any set $A$, $A \cap \emptyset = \emptyset$.
